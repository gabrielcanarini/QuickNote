                ...
                <PopoverContent className="w-80">
                  <div className="grid gap-4">
                    <div className="space-y-2">
                      <h4 className="leading-none font-medium">Nuovo tipo</h4>
...
//funzioni per gestire i tipi sul db
type OptionType = {
  label: string;
  value: string;
  icon?: React.ComponentType<{ className?: string }>;
...
async function deleteOption(id: string) {
  const supabase = createClient();
  await supabase.from("note_types").delete().eq("id", id);
  mutate("typeUser");
}

const getType = async (): Promise<OptionType[]> => {
  const supabase = createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
